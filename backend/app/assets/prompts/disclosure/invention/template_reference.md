...
source_path: prompts/disclosure/invention/template_reference.md
source_url: https://github.com/handsomestWei/patent-disclosure-skill
treatment: verbatim
ported_version: 2026-08-25.1
notes: 整体逐字复制（六章模板+文头+mermaid 模版+公式正反例+脱敏检查表+标题贯穿检查+交付正文禁忌）。文中少量行提及原仓库脚本/文件名（mermaid_render.py、formula_plan.yaml、§7.3 等），按 verbatim 处置保留；common/system_base.md 已约束「规则文本中提及的脚本/路径仅为体例说明，不得写入产出正文」，另有服务端 lint 兜底。成文各调用按需注入。
---
