...
source_path: prompts/disclosure/utility_model/patent_points.md
source_url: https://github.com/handsomestWei/patent-disclosure-skill
treatment: adapted
ported_version: 2026-08-25.1
notes: 结构点挖掘要求表与「多图关系→挖点启发式」表逐字保留；fill_structure_schema 落盘前提改为平台注入；not_utility_model_signals 反问改为 JSON 字段 + 平台反问；「下一步 Read/--type」删除；新增流式 markdown + 尾部 JSON 候选契约（2-4 个候选）。
---

# 实用新型 · 专利点挖掘（Step 3–4 特化）

**本文件仅用于专利类型 = 实用新型。** 发明 / 外观的类型路由由平台处理。

平台已注入本案 **StructureSchema** 与 **figure_plan**（填表阶段产出）。挖点叙述须能回溯到 schema 的 `parts` / `relations` / `spatial`，并**主动联读** `figure_plan.relates_to`（勿只当入文清单）。
...
若 `not_utility_model_signals` 非空（如纯算法、纯外观美感、方法步骤为主）：在输出 JSON 的 `type_mismatch_note` 说明并建议改判（平台将反问用户），勿硬写实用新型。
...
1. 工作标题（「一种……装置/结构」）  
2. StructureSchema 定稿（平台留存）  
3. **figure_plan**（入文图按 score 排序；与 `parts`/`covers` 一致；支撑选定点的图际链 `relates_to` 已写；主题变了须重评）

（查新由平台以类型 `utility_model` 执行。）

## 输出契约

先**流式输出候选结构点分析**（markdown，按上表各项展开），随后在**文末**输出一个 ```json 围栏块：
  ...
  "candidates": [
    {
      "id": "P1",
      "title": "工作标题（「一种……装置/结构」）",
      "background": "技术背景：现有安装/连接/布局方式及痛点",
      "innovation": "构造创新：对应的部件形状/连接关系/空间布局（引用件号或 schema 字段），含图证 fig 链",
      "distinction": "与现有技术区别（查新前可据材料推断）",
      "feasibility": "可实施性；uncertain 项如实说明",
      "score": 0
    }
  ],
  "recommended": "P1",
  "fusion_note": "融合建议（同一 relates_to 链的候选默认合成一篇：总装定范围、局部定区分特征）；无则空字符串",
  "type_mismatch_note": "not_utility_model_signals 非空时的改判建议；无则空字符串"
}
```

规则：候选 **2–4 个**（结构向，勿默认「方法」）；`score` 0–100（构造可区分性、可实施、装置书式保护范围、图证链闭合）；`recommended` 为建议选定候选 id。
